import type { FilterCondition, FilterRule, ParsedColor, ParsedFilter } from "./types";
...
function parseRule(data: Uint8Array): FilterRule {
  ...
  const conditions = F.filter((x) => x.f === 4).flatMap((ff) => {
    const raw = ff.t === "b" ? (ff.v as Uint8Array) : (ff as Field).raw;
    if (!raw) return [];
    try {
...
export function parseFilterB64(b64: string): ParsedFilter {
  ...
  return { name, rules, topLevelFlags: topLevelFlags.length ? topLevelFlags : undefined };
}
